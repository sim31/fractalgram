...
addActionHandler('loadRemainingMessages', async (global, actions, payload): Promise<void> => {
...
async function loadWithBudget<T extends GlobalState>(
  ...
  if (!areAllLocal) {
    await loadViewportMessages(
      global, chat, threadId, offsetId, direction, isOutlying, isBudgetPreload, MESSAGE_LIST_SLICE, onLoaded, tabId,
    );
  }
...
async function sendMessageImpl(
  ...
  if (isSingle) {
    ...
    await sendMessageOrReduceLocal(global, sendParams, localMessages, sentCb);
  } else if (isGrouped) {
    const {
...
addActionHandler('sendPinnedMessage', (global, actions, payload): ActionReturnType => {
  ...
  return sendMessageImpl(global, actions, payload, (messageId) => {
    actions.pinMessage({
      chatId, messageId, isUnpin: false, isSilent: true
    });
  })
});
...
async function sendMessageOrReduceLocal<T extends GlobalState>(
  ...
  if (!sendParams.messagePriceInStars) {
    sendMessage(global, sendParams, sentCb);
  } else {
...
async function sendMessage<T extends GlobalState>(global: T, params: SendMessageParams, messageSentCb?: MessageSentCb) {
  // @optimization
